...
class MyGUI:
    def __init__(self, data):
        ...
        if 'dHdkx' in data and 'Dxk' in data:
            ...
            showDipole = 'Dxk' in data
            showDerivative = 'dHdx' in data

        if showDipole:
            if 'Dzk' in data:
                self.operators = np.stack((data['H0'], data['Dxk'], data['Dyk'], data['Dzk']))
            else:
                self.operators = np.stack((data['H0'], data['Dxk'], data['Dyk'], np.zeros(data['H0'].shape, dtype=complex)))
        ...
        if showDerivative:
            if 'dHdkz' in data:
                ...
                self.operators = np.stack((data['H0'], data['dHdkx'], data['dHdky'], np.zeros(data['H0'].shape, dtype=complex)))
        ...
        opCount, self.Nk1, self.Nk2, self.Nk3, _, _, = self.operators.shape
        ...
        self.fig.subplots_adjust(bottom=0.25, left=0.05, right=0.98, top=0.98, wspace=0.35, hspace=0)
...
if __name__ == "__main__":
    typeMap = { 'direct' : 'trafoDirect.npz' , 'fft': 'trafoFFT.npz', 'python' : 'trafoPython.npz',
