...
The pulse used for the propagation and its contributions can be written to a file using the **Pulse** run mode.
In the pulse section, all parameters ending without a number are set the default values.
When a parameter is padded by a number, a new pulse contribution will be created with all the default parameters and the number specific ones.
