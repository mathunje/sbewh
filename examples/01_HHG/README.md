...
We can calculate the time dependent expectation values for silicon in a 1D projected model, via
...
Most of the visualization scripts parse the complete run directory. If the givn directory contains no input file, they will process the latest subdirectory (according to the numbering).
...
An example for a 3D calculation is given in **input3D.txt**.
