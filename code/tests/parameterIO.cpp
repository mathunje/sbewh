...
TEST(ParameterIO, validFile){
    ...
    EXPECT_EQ(p.parse("../tests/data/inputValid.txt"), ParameterIO::ParseResult::Ok);
    // p.printParam(stdout);
    EXPECT_EQ(a, 123248);
    EXPECT_EQ(b, 12);
    EXPECT_EQ(c, 42);
...
TEST(ParameterIO, invalidFile){
    ...
    p.finishGroup();
    EXPECT_EQ(p.parse("../tests/data/inputInvalid.txt"), ParameterIO::ParseResult::Fail);
...
TEST(ParameterIO, validArgv){
    ...
    bool help = false;
    EXPECT_EQ(p.parse(argc, args, "../tests/data/emptyInput.txt"), ParameterIO::ParseResult::Ok);
    EXPECT_EQ(a, 12);
}
...
TEST(ParameterIO, loadFileArgv){
    int a = 0, b = 0 , c = 0, csg = 0, d = 0;
    ParameterIO p = paramIO(a, b, c, csg, d);
    const char * args[] = { "dummyname", "../tests/data/inputValid.txt", "-c=-1", NULL };
    int argc = sizeof(args)/sizeof(args[0]) - 1;
